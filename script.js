...
function fetchInfo() {
    ...
    var requestUrl = 'https://api.openweathermap.org/geo/1.0/direct?q=' + location + '&limit=5&appid=' + api;
    $('#location').val("");
    fetch(requestUrl)
        ...
        .then(function (response) {
            if (response.ok) {
                response.json().then(function (data) {
                    ...
                    var forecastDate = document.getElementById('todayDate');
    ...
    fetch(requestUrl)
        ...
        .then(function (data) {
            ...
            for (i = 7, j = 0; i < 40, j < 5; i += 8, j++) {
                ...
                weatherImage.src = "https://openweathermap.org/img/wn/" + data.list[i].weather[0].icon + ".png"
                weatherImage.alt = "demo icon"
                ...
                $('#todayDate').text('Today :   ' + moment(data.list[0].dt_txt, 'YYYY-MM-DD hh:mm:ss').format("MMM Do, YYYY"))
